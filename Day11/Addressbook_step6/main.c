...
// step5
static int save_contacts(const char* filename);	// const 붙은 이유: 파일명이 변경되면 안 됨
static int load_contacts(const char* filename);	
static void trim_newline(char* s);	// 줄바꿈 제거 기능 추가

// step6
static int contains_pipe(const char* s);	// 입력 문자중에 | 존재 확인
...
static void delete_contact(void) {
	...
	while ((ch = getchar()) != '\n' && ch != EOF) {}
		...
		if (!(yn[0] == 'y' || yn[0] == 'Y')) { // 소문자 y 이거나 대문자 Y가 아니면
			puts("삭제를 취소합니다.");
			return;

		}

		// 현재 삭제할 값 이후의 배열요소를 앞으로 당김
		for (i = index - 1; i < count - 1; i++) {
			contacts[i] = contacts[i + 1];	// 전부 앞으로..
		}
		count--;	// 한 건 지웠으니 전체 수를 1 감소
...
static int load_contacts(const char* filename) {
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		// contacts 배열에 하나씩 대입
		if (!name || !phone || !email || !memo || !address) continue;	// 다섯 개 데이터 중 하나라도 없으면 패스

		if (count >= MAX_CONTACTS) break;	// 현재 100개밖에 못 넣음
		...
		contacts[count].phone[PHONE_LEN - 1] = '\0';	// 번호 길이가 너무 길면 정리
